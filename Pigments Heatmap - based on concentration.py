# -*- coding: utf-8 -*-
"""
Created on Wed Mar 12 12:47:29 2025
...
# Convert 'Condition' and 'Time' columns to strings
...
# Dynamically drop the condition columns after one-hot encoding
condition_columns = [col for col in data.columns if col.startswith('Condition')]

# Dropping columns dynamically based on condition column names
X = data.drop(columns=condition_columns)  # Dropping condition columns from X

# Create target variable 'y' using one-hot encoded condition columns
y = data[condition_columns]
...
# Visualize the average confusion matrix using a heatmap
plt.figure(figsize=(10, 8))  # Larger figure size for better readability
ax = sns.heatmap(avg_conf_matrix, annot=False, cmap='Blues', 
                 xticklabels=label_encoder.classes_, yticklabels=label_encoder.classes_,
                 cbar_kws={'label': 'Number of predictions'}, linewidths=0.5, linecolor='gray')

# Annotate the confusion matrix cells with the exact values
for i in range(avg_conf_matrix.shape[0]):
    for j in range(avg_conf_matrix.shape[1]):
        ax.text(j + 0.5, i + 0.5, f'{avg_conf_matrix[i, j]:.2f}',  # Use exact float values
...
plt.savefig('Pigments_Average_Confusion_Matrix.png', dpi=300) 
plt.show()

# Saving the DataFrame to Excel
with pd.ExcelWriter('Data_Frame_New.xlsx') as writer:
